...
Restart `dsh web` afterwards. The "自动工作" (Auto Work) entry then appears in the sidebar.

## Usage

1. Open the "自动工作" sidebar panel.
...
4. "立即执行" (Run now) triggers a run immediately (stop time = now + original duration).
...
- Sessions are titled "[自动] <goal prefix>" so they are easy to find in the session list.
