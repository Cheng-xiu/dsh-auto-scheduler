...
try {
  ...
  console.warn("[dsh-auto-scheduler] 读取 package.json 版本失败，使用默认版本 " + VERSION + ": " + String(error && error.message ? error.message : error));
}
...
// Retry cap for failed session starts: after this many consecutive failures within
// one occurrence window the schedule is parked as "error" until the next window or
// manual re-enable, instead of retrying on every tick for the whole window.
...
// Move an unreadable store file aside (with a timestamped name) so falling back
// to defaults cannot silently destroy user data.
let backupCounter = 0;
function backupCorruptStore(reason) {
  const file = storePath();
  const makeBackupName = () => {
    backupCounter += 1;
    return join(dirname(file), "dsh-auto-scheduler.corrupt-" + Date.now() + "-" + backupCounter + "-" + Math.random().toString(36).slice(2, 8) + ".json");
  };
  let backup = makeBackupName();
  try {
    renameSync(file, backup);
    console.warn("[dsh-auto-scheduler] store 文件不可用（" + reason + "），已备份为 " + backup + "，改用空配置");
    return;
  } catch (error) {
    // Retry once with a different name in case the first target collided.
    try {
      backup = makeBackupName();
      renameSync(file, backup);
      console.warn("[dsh-auto-scheduler] store 文件不可用（" + reason + "），已备份为 " + backup + "，改用空配置");
      return;
    } catch (retryError) {
      console.warn("[dsh-auto-scheduler] store 文件不可用（" + reason + "），备份失败: " + errMsg(retryError) + "，改用空配置");
    }
  }
}
...
function loadStore() {
  ...
  try {
    ...
    if (!error || error.code !== "ENOENT") console.warn("[dsh-auto-scheduler] 读取 store 失败: " + errMsg(error));
    return defaultStore();
  }
  let parsed;
  try {
...
async function startSession(ctx, s) {
  ...
  const sessionId = createResult.value ? createResult.value.sessionId : undefined;
  if (!sessionId) throw new Error("创建会话失败: 返回结果缺少 sessionId");
  ...
  try {
    ...
    try {
      ...
      if (ctx.logger) ctx.logger.warn("dsh-auto-scheduler: 重命名会话失败: " + errMsg(error));
    }
    ...
    // A later step (permission/rename/prompt) failed: cancel the just-created
    // session so it is not left orphaned, then propagate the error.
    try {
      await ctx.apiProxy.sessions.cancel({ rpcId: rpcId(), payload: { sessionId: sessionId } });
      s.sessionId = null;
    } catch (cancelError) {
      if (ctx.logger) ctx.logger.warn("dsh-auto-scheduler: 启动失败后取消会话 " + sessionId + " 失败: " + errMsg(cancelError));
    }
    throw error;
  }
...
// ---------- scheduler tick ----------
let ticking = false; // re-entrancy guard: a tick still in flight blocks the next one
let tickStartedAt = 0;
const TICK_TIMEOUT_MS = 5 * 60 * 1000; // safety valve: a hung RPC must not freeze the scheduler
const startFailures = new Map(); // schedule id -> { occ, count } consecutive failed starts for the current occurrence

async function tick(ctx) {
  if (ticking) {
    if (Date.now() - tickStartedAt > TICK_TIMEOUT_MS) {
      console.warn("dsh-auto-scheduler: tick 超过 5 分钟未完成，强制重置守卫");
      ticking = false;
    } else {
      return; // previous tick not finished yet; skip to avoid double-firing
    }
  }
  ticking = true;
    ...
    for (const s of store.schedules) {
      ...
      if (s.status === "running") {
        ...
        if (!Number.isFinite(startMs) || !Number.isFinite(stopMs) || !Number.isFinite(lastRun)) {
          // Cannot compute a stop time from the recorded timestamps: stop the session
          // and flag the schedule instead of leaving it "running" forever.
          await stopSession(ctx, s, "时间信息无效");
          const live = store.schedules.find((x) => x.id === s.id);
          if (!live) continue; // deleted while awaiting; drop it
          live.status = "error";
          live.lastError = "时间信息无效，已强制停止";
          changed = true;
          continue;
        }
      ...
      if (occ) {
        ...
        // Mark fired BEFORE the await so an overlapping tick cannot double-fire this occurrence.
        ...
        try {
          ...
          startFailures.delete(s.id);
          const live = store.schedules.find((x) => x.id === s.id);
          if (!live) continue; // deleted while awaiting
          live.lastRunAt = new Date().toISOString();
          live.status = "running";
          live.lastError = null;
        } catch (error) {
          // Bounded retries: count consecutive failures within this occurrence window;
          // once MAX_START_FAILURES is reached, leave lastFiredAt set so we stop
          // retrying until the next window or manual re-enable (see toggle handler).
          const prev = startFailures.get(s.id);
          const count = prev && prev.occ === occ.start ? prev.count + 1 : 1;
          startFailures.set(s.id, { occ: occ.start, count: count });
          const live = store.schedules.find((x) => x.id === s.id);
          if (!live) continue;
          live.status = "error";
          live.lastError = errMsg(error);
          if (count < MAX_START_FAILURES) live.lastFiredAt = null; // allow retry on next tick
        }
        continue;
      }
      if (s.repeat === "once") {
...
async function readJsonBody(req) {
  ...
  try {
    ...
    return typeof parsed === "object" && parsed !== null ? { body: parsed } : {};
  } catch (error) { return {}; }
}
  ...
  return [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        const readBody = await readJsonBody(req);
        if (readBody.tooLarge) { writeJson(res, 413, { ok: false, error: "body too large" }); return; }
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        if (s.enabled && s.status === "error") {
          // Manual re-enable lifts the failure cap so the schedule may fire again.
          s.status = "idle";
          s.lastFiredAt = null;
          s.lastError = null;
          startFailures.delete(s.id);
