...
function backupCorruptStore(reason) {
  ...
  const backup = join(dirname(file), "dsh-auto-scheduler.corrupt-" + Date.now() + ".json");
  try {
    renameSync(file, backup);
    console.warn("[dsh-auto-scheduler] store 文件不可用（" + reason + "），已备份为 " + backup + "，改用空配置");
  } catch (error) {
    console.warn("[dsh-auto-scheduler] store 文件不可用（" + reason + "），备份失败: " + errMsg(error) + "，改用空配置");
  }
}
...
async function tick(ctx) {
  if (ticking) return; // previous tick not finished yet; skip to avoid double-firing
  ticking = true;
  try {
    const now = Date.now();
    let changed = false;
    for (const s of store.schedules) {
      ...
      if (occ) {
        ...
        try {
          ...
          if (count < MAX_START_FAILURES) s.lastFiredAt = null; // allow retry on next tick
        }
        continue;
      }
      if (s.repeat === "once") {
