			...
			try {
				...
				for (var j = 0; j < segs.length; j++) {
					...
					try {
						...
						segs[j].ok = true;
						if (j === 0 && editId) { editId = null; updateEditHint(); } // edit applied; later rows are new records
					} catch (err) {
						failed++;
				...
				if (failed > 0) {
					// Drop the rows that saved successfully so retrying only re-posts the failed segments.
					for (var k = segs.length - 1; k >= 0; k--) {
						if (segs[k].ok && rows[k] && rows[k].parentElement) rows[k].parentElement.removeChild(rows[k]);
					}
